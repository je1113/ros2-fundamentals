...
- **`/World/YawPivot`**: 질량 있는(robot1과 비슷한 3.5kg — 아래 5절 참고) 충돌 없는 더미 RigidBody. `/World/YawPivot/RevoluteJoint`로 **월드 고정 프레임**(body0 비워둠)에 Y축 Revolute Joint + Angular Drive로 연결 — robot1의 회전(yaw)을 담당.
- **`/World/robotvacum_decimated/robot1`**: `/World/robotvacum_decimated/robot1/PrismaticJoint`로 YawPivot에 X축 Prismatic Joint + Linear Drive로 연결. 이 Joint의 X축은 YawPivot 로컬 프레임 기준이라 **YawPivot이 돌면 같이 돈다** — 그래서 "지금 향하고 있는 방향으로 전진"이 자동으로 성립. (경로 표기 주의: 처음엔 `/World/YawRevoluteJoint`/`/World/ForwardPrismaticJoint`로 직접 스크립트 생성했었는데, Topic 7 세션에서 GUI(`Create > Physics > Joint`)로 다시 만들면서 위 경로로 바뀌었다 — 자세한 경위는 5절.)
- Prismatic Joint가 YawPivot 기준 X축 이동 1자유도만 남기고 나머지(Y/Z 이동, 3축 회전)를 전부 잠그는 구조라, robot1은 지형을 따라가지 않고 YawPivot 높이에서 이동한다 — 바닥에 붙어서 구르는 느낌을 원하면 나중에 별도로 다듬어야 함(예: Y축에 살짝 여유를 주는 스프링/댐퍼 Joint 추가).
...
| **(Topic 7 세션에서 추가 발견)** Script Editor로 만든 `ForwardPrismaticJoint`/`YawRevoluteJoint`는 매번 Play할 때마다 `PhysicsUSD: CreateJoint - found a joint with disjointed body transforms` 경고가 뜨고, robot1이 YawPivot 기준 Y축(잠겨 있어야 할 축)으로 자유낙하 — GUI에서 두 프림의 Translate를 정확히 일치시켜도 재현됨 | 원인 특정은 못 했지만(수동으로 계산한 `LocalPos0`/`LocalPos1`이 아주 미세하게 안 맞았거나, `PhysicsPrismaticJoint`/`PhysicsRevoluteJoint` 프림을 `stage.DefinePrim()`으로 직접 만드는 방식 자체가 뭔가 놓친 초기화가 있는 듯) | 기존 Joint를 지우고 **Isaac Sim의 `Create > Physics > Joint > Prismatic/Revolute Joint` 메뉴로 다시 생성**(두 프림을 선택한 상태에서 실행하면 현재 위치 기준으로 로컬 프레임을 자동 계산해줌) — 이렇게 만드니 완전히 해결됐음. **교훈: Joint의 로컬 프레임 계산은 손으로 하지 말고 GUI 생성에 맡길 것.** 단, GUI로 새로 만들면 프림이 선택했던 오브젝트 하위에 생성되는 등 경로가 바뀌므로(`/World/robotvacum_decimated/robot1/PrismaticJoint`, `/World/YawPivot/RevoluteJoint`), `CmdVelGraph`의 `WriteLinearDrive`/`WriteAngularDrive` 노드의 `Prim Path`도 새 경로로 맞춰줘야 함. 추가로 `YawPivot`의 질량(원래 0.1kg, robot1과 35배 차이)이 극단적으로 가벼워서 아주 미세한 잔류 드리프트(초당 0.0001 단위)가 있었는데, `YawPivot` 질량을 robot1과 비슷하게(3.5kg) 올리자 실질적으로 사라짐 — 관절로 연결된 두 바디의 질량 비가 너무 크면 솔버가 불안정해질 수 있다는 것도 기억해둘 것 |
...
- [x] cmd_vel → Joint Drive 배선, `ros2 topic pub /cmd_vel`/teleop 둘 다로 방향 전환 포함 정상 구동 확인 (Topic 7 세션에서 최종 안정화, 아래 5절 추가 항목 참고)
